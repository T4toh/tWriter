...
- [tWriter](#twriter)
  ...
  - [Features](#features)
    ...
    - [Estado de la novela + revisiones](#estado-de-la-novela--revisiones)
...
- **La puntuación filtra, no solo ranquea**: `—dijo` devuelve **solo** los capítulos que tienen la raya pegada al verbo, no todos los que tienen un `dijo` suelto (que con el boost de arriba igual venían, abajo en la lista). El caso real es cazar los dicendi vacíos de los primeros borradores: `—dijo`, `—preguntó`, `—respondió` uno por uno. Si ningún capítulo tiene el literal, vuelven los hits por palabra con el aviso «tal cual no está» (`matchLevel: noLiteral`), porque adentro del editor no se va a resaltar nada. Solo dispara con puntuación, no con mayúsculas: `Kellai` en fuzzy no está literal en ningún lado y es un hit válido. Y el salto desde el resultado usa el literal en vez de `matchedTerms` cuando hay puntuación, o caía en el primer `dijo` pelado del capítulo.
...
- **Visor de imágenes con zoom** en el panel derecho: la imagen arranca
  encajada (nunca agrandada). Rueda o pinch hace zoom alrededor del cursor,
  `⌘/Ctrl +`, `-` y `0` acercan, alejan y vuelven a encajar, arrastrar panea
  cuando la imagen excede el viewport, doble click alterna encaja / 1:1, y
  el porcentaje del header vuelve a encajar. La matemática (encaje, zoom
  alrededor de un punto, clamp del pan) es pura en `image-viewer/zoom.ts`
  con smoke en `scripts/run-zoom-smoke.mjs`.
...
### Estado de la novela + revisiones

- `book.json::estado` recorre **en curso → terminada → publicada**. Desde
  `terminada` se esconde el creador de capítulos, partes y epílogo — que es
  todo lo que sabía hacer el `finalizada: bool` viejo. La migración es al leer
  (`true` → `terminada`, ausente → `en curso`) y el campo viejo no se vuelve a
