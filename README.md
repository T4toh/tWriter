...
- **Bug latente: race entre `StorageService.detect` async y `GitService.effect`**. El `effect()` en `git-service.ts:87-105` lee `storage.isGit()` al boot, pero `StorageService.detect` (`storage-service.ts:55-64`) corre `detect_storage_backend` de forma asíncrona — el effect arranca con `isGit=false`, entra al `else`, deja timers apagados y status en `null`. Cuando `backend` se vuelve `'git'` después del invoke, el effect **debería** re-correr (es signal-based) pero en pruebas (rama `feat/git-fetch-on-open`, ya borrada) la UI quedó en "sin estado" indefinidamente con el panel de debug vacío y sin warnings en consola — el if-branch nunca entró. Backend Rust verificado sano: `git_status` y `detect_storage_backend` devuelven datos correctos cuando se invocan directo via `window.__TAURI_INTERNALS__.invoke` desde devtools. Fix probable en 1-3 líneas envolviendo el `void this.initialSync()` + `startTimers()` con una guarda que espere a `backend !== 'unknown'`, o forzando `storage.refresh()` antes de evaluar `isGit()`. Bloquea cualquier feature nueva que toque sync (`fetch-on-open`, periodic fetch, pre-push fetch). Próximo paso de debug: agregar `console.log({root, isGit, backend})` al inicio del effect para ver la secuencia real de re-runs.
- **Fetch silencioso al abrir + pre-push** (diferido por el bug de arriba): que la app traiga refs remotos al cargar el root y antes de cada auto-push, en vez de depender del recovery por non-FF rejection. Espejo de `git_pull_impl` con `["fetch", "--prune"]` + hook en el effect de `GitService` después de `git_ensure_twriter_ignored` y al inicio de `syncNow()`. Filosofía: "no te hagas el lindo" — fallar silencioso si no hay agent SSH, el editor sigue offline-funcional.
